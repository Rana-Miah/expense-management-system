...
export const LoanFinanciersRestoreCards = ({ items, revalidatePathname }: { items: { id: string, label: string }[], revalidatePathname?: string }) => {

    const onClose = useAlertModalClose()
    const { isAlertOpen, payload } = useAlertModal<RestoreAlertModalPayload>()
    const [pending, startTransition] = useTransition()
    const isOpen = !!payload && isAlertOpen && payload.modalType === RESTORE_MODAL_TYPE.RESTORE_LOAN_FINANCIER
                    ...
                    items.map(
                        ({ id, label }) => (
                            <RestoreCard
                                id={id}
                                label={label}
                                modalType={RESTORE_MODAL_TYPE.RESTORE_LOAN_FINANCIER}
                                key={id}
                            />
                        )
                    )
