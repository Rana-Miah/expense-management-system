...
export const assignSourceTable = pgTable("assign_source", {
    id: uuid('id').primaryKey().unique().defaultRandom(),
    clerkUserId: text('clerk_user_id').notNull(),
    sourceBankId: uuid('source_bank_id').notNull().references(() => bankAccountTable.id,),
...
export const assignSourceTableRelation = relations(assignSourceTable, ({ one }) => ({
    sourceBank: one(bankAccountTable, {
        ...
        relationName:relationBetween('assign_source','bank')
    }),
    transactionName: one(trxNameTable, {
        fields: [assignSourceTable.trxNameId],
        references: [trxNameTable.id],
        relationName:relationBetween('assign_source','transaction_name')
    }),
}))

export const assignReceiveTable = pgTable("assign_receive", {
    id: uuid('id').primaryKey().unique().defaultRandom(),
    clerkUserId: text('clerk_user_id').notNull(),
...
export const assignReceiveTableRelation = relations(assignReceiveTable, ({ one }) => ({
    receiveBank: one(bankAccountTable, {
        ...
        references: [bankAccountTable.id],
        relationName:relationBetween('assign_receive','bank')
    }),
    transactionName: one(trxNameTable, {
        fields: [assignReceiveTable.trxNameId],
        references: [trxNameTable.id],
        relationName:relationBetween('assign_receive','transaction_name')
    }),
}))